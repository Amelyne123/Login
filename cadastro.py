from kivy.app import App
from kivy.uix.boxlayout import BoxLayout
from kivy.uix.label import Label
from kivy.uix.textinput import TextInput
from kivy.uix.button import Button
from kivy.core.window import Window
from kivy.uix.image import Image

class TelaCadastro(BoxLayout):
    def __init__(self, **kwargs):
        super(TelaCadastro, self).__init__(**kwargs)
        self.orientation = "vertical"
        self.padding = [50, 20]
        self.spacing = 10

        Window.clearcolor = (0.15, 0.15, 0.15, 1)
        self.add_widget(Image(source='/Users/aluno.sesipaulista/Desktop/Login/img/icon_img.png'))    

        self.add_widget(Label(text='CADASTRO', font_size=24, color=(0, 0, 0, 1)))

        self.email_input = TextInput(hint_text='E-mail', multiline=False)
        self.password_input = TextInput(hint_text='Senha', password=True, multiline=False)
        self.add_widget(self.email_input)
        self.add_widget(self.password_input)

        self.buttons_layout = BoxLayout(padding=[0, 10], spacing=10)
        self.cadastrar_button = Button(text='Cadastrar', color=(0, 0, 0, 1), size_hint=(None, None), size=(450, 50),
                                       background_color=(100, 100, 100, 100))
        self.cadastrar_button.bind(on_press=self.cadastrar)
        self.voltar_button = Button(text='Voltar para o Login', color=(0, 0, 0, 1), size_hint=(None, None), size=(450, 50),
                                    background_color=(100, 100, 100, 100))
        self.voltar_button.bind(on_press=self.voltar_para_login)
        self.buttons_layout.add_widget(self.cadastrar_button)
        self.buttons_layout.add_widget(self.voltar_button)
        self.add_widget(self.buttons_layout)

    def cadastrar(self, instance):
        email = self.email_input.text
        password = self.password_input.text
        print('Novo usuário cadastrado:')
        print('E-mail:', email)
        print('Password:', password)

    def voltar_para_login(self, instance):
        self.clear_widgets()
        self.add_widget(Label(text='LOGIN', font_size=24, color=(0, 0, 0, 1)))

        self.email_input = TextInput(hint_text='E-mail', multiline=False)
        self.password_input = TextInput(hint_text='Senha', password=True, multiline=False)
        self.add_widget(self.email_input)
        self.add_widget(self.password_input)

        self.buttons_layout = BoxLayout(padding=[0, 10], spacing=10)
        self.login_button = Button(text='Login', color=(0, 0, 0, 1), size_hint=(None, None), size=(450, 50),
                                   background_color=(100, 100, 100, 100))
        self.login_button.bind(on_press=self.login)
        self.cadastro_button = Button(text='Cadastre-se', color=(0, 0, 0, 1), size_hint=(None, None), size=(450, 50),
                                      background_color=(100, 100, 100, 100))
        self.cadastro_button.bind(on_press=self.abrir_tela_cadastro)
        self.buttons_layout.add_widget(self.login_button)
        self.buttons_layout.add_widget(self.cadastro_button)
        self.add_widget(self.buttons_layout)

    def login(self, instance):
        email = self.email_input.text
        password = self.password_input.text
        print('Email:', email)
        print('Password:', password)

    def abrir_tela_cadastro(self, instance):
        self.clear_widgets()
        self._init_()

class MyApp(App):
    def build(self):
        return TelaCadastro()

if __name__ == '__main__':
    MyApp().run()